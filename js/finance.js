...
/** Redondea hacia arriba al múltiplo de `multiple` más cercano. */
function roundUpToMultiple(value, multiple) {
  if (!Number.isFinite(value)) return 0;
  return Math.ceil(value / multiple) * multiple;
}

/** Todo monto en pesos se redondea SIEMPRE hacia arriba, en múltiplos de $1000. */
function roundARS(value) {
  return roundUpToMultiple(value, 1000);
}

/** Todo monto en dólares se redondea SIEMPRE hacia arriba, en múltiplos de US$5. */
function roundUSD(value) {
  return roundUpToMultiple(value, 5);
}

/** Formatea un número como pesos argentinos, redondeado a múltiplo de $1000: $1.430.000 */
function formatARS(value) {
  return "$" + roundARS(value).toLocaleString("es-AR", { maximumFractionDigits: 0 });
}

/** Formatea un número como dólares, redondeado a múltiplo de US$5: US$ 950 */
function formatUSD(value) {
  return "US$ " + roundUSD(value).toLocaleString("es-AR", { maximumFractionDigits: 0 });
}

/** Formatea una tasa/cotización en pesos SIN forzar redondeo a múltiplo de 1000
 *  (para mostrar, tal cual, el valor del dólar que cargó el usuario). */
function formatARSRate(value) {
  if (!Number.isFinite(value)) return "$0";
