...
const state = {
  ...
  meta: null,
  quote: {
    items: [], // {productId, name, qty, costUsd, markupUsd, financingType, ownInstallmentsSelected, cardInstallmentsSelected}
    dollarRate: 0,
    ownTem: 5,
    cardRate6: 57,
    cardRate12: 91,
...
function itemCardHTML(item) {
  ...
  const finTypes = [
    { type: "none", label: "Sin financiación", cls: "" },
    { type: "own", label: "Propia", cls: " fin-type-btn--own" },
    { type: "card", label: "Tarjeta", cls: " fin-type-btn--card" },
  ];
  const finButtons = finTypes
    .map(
      ({ type, label, cls }) =>
        `<button type="button" class="fin-type-btn${cls}${item.financingType === type ? " is-selected" : ""}" data-fintype="${item.productId}" data-type="${type}">${label}</button>`
    )
    .join("");

  let finBody = "";
  if (item.financingType === "own") {
  ...
  } else if (item.financingType === "card") {
...
function generateQuote() {
  ...
  for (const item of items) {
    if (item.financingType === "own" && !item.ownInstallmentsSelected.length) {
      toast(`Elegí al menos una cuota para "${item.name}" (financiación propia)`);
      return;
    }
    if (item.financingType === "card" && !item.cardInstallmentsSelected.length) {
  ...
  const message = buildWhatsAppMessage(payloadItems, state.quote.dollarRate, state.meta.businessName);
...
function wireQuote() {
  ...
  $("quote-items").addEventListener("click", (e) => {
    ...
    const fintypeBtn = e.target.closest("[data-fintype]");
    ...
    if (fintypeBtn) {
      const id = fintypeBtn.dataset.fintype;
      const type = fintypeBtn.dataset.type;
      const item = state.quote.items.find((it) => it.productId === id);
      if (item) {
        item.financingType = type;
        renderQuoteItems();
      }
      return;
    }
